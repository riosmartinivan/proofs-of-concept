package com.phorus.petservice.model.dtos

import com.phorus.petservice.model.validationGroups.PetCreateValGroup
import com.phorus.petservice.model.validationGroups.PetUpdateValGroup
import org.junit.jupiter.api.Test
import javax.validation.Validation
import javax.validation.Validator
import kotlin.test.assertEquals
import kotlin.test.assertTrue
import kotlin.test.fail

class PetDTOTest {

    private val validator: Validator = Validation.buildDefaultValidatorFactory().validator

    @Test
    fun `test update validations with errors`() {
        val petDTO = PetDTO("ab", -1)

        val violations = validator.validate(petDTO, PetUpdateValGroup::class.java)

        assertEquals(2, violations.size)

        violations.forEach {
            when (it.propertyPath.toString()) {
                "name" -> assertEquals("Must have at least 3 characters long", it.message)
                "age" -> assertEquals("Cannot be negative", it.message)
                else -> fail("Unknown violation")
            }
        }
    }

    @Test
    fun `test update validations without errors`() {
        val petDTO = PetDTO()

        val violations = validator.validate(petDTO, PetUpdateValGroup::class.java)

        assertEquals(0, violations.size)
    }

    @Test
    fun `test create validations with errors`() {
        val petDTO = PetDTO("")

        val violations = validator.validate(petDTO, PetCreateValGroup::class.java)

        assertEquals(4, violations.size)

        violations.forEach {
            when (it.propertyPath.toString()) {
                "name" -> assertTrue(
                    it.message == "Cannot be blank" ||
                            it.message == "Must have at least 3 characters long",
                    "Unexpected name violation value: " + it.message
                )
                "age" -> assertEquals("Cannot be null", it.message)
                "userId" -> assertEquals("Cannot be null", it.message)
                else -> fail("Unknown violation")
            }
        }
    }

    @Test
    fun `test create validations without errors`() {
        val petDTO = PetDTO("abc", 1, 1L)

        val violations = validator.validate(petDTO, PetCreateValGroup::class.java)

        assertEquals(0, violations.size)
    }
}